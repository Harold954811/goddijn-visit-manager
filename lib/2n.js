...
// Build an ISO timestamp for a given date at a specific local time in the
// Europe/Paris timezone (CET/CEST — covers Amsterdam, Mougins, Courchevel,
// Rome). The 2N box stores times in local time, so we must send local time
// with the correct UTC offset, not bare UTC.
//
// The offset is determined by checking whether the date falls in CET
// (UTC+1, last Sun Oct → last Sun Mar) or CEST (UTC+2, last Sun Mar →
// last Sun Oct). This avoids depending on a timezone library.
function cetIsoTimestamp(dateStr, hhmmss) {
  // Parse the date to determine CET vs CEST
  const d = new Date(dateStr + "T12:00:00Z");
  const year = d.getUTCFullYear();
  // Last Sunday of March and October (CEST starts/ends)
  ...
  const isCEST = d >= lastSunMar && d < lastSunOct;
  const offset = isCEST ? "+02:00" : "+01:00";
  return `${dateStr}T${hhmmss}${offset}`;
}
...
export async function createVisitor({ guestName, guestEmail, startDate, endDate, pin, twoNGroupId, groupName }) {
  ...
  const body = {
    ...
    VisitFrom: cetIsoTimestamp(startDate, "05:00:00.000"),
    VisitTo: cetIsoTimestamp(endDate, "23:59:59.000"),
