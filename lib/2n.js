...
export async function updateVisitor({ visitorId, startDate, endDate, twoNGroupId, groupName }) {
  ...
  if (twoNGroupId) {
    patch.Groups = [{ Id: twoNGroupId, Name: groupName || "" }];
  }
  ...
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`2N update visitor failed (${res.status}): ${text.slice(0, 300)}`);
  }
...
export async function deactivateVisitor(visitorId) {
  ...
  if (!res.ok && res.status !== 404) {
    const text = await res.text();
    throw new Error(`2N deactivate visitor failed (${res.status}): ${text.slice(0, 300)}`);
  }
}
