...
// Revokes a 2N visitor's access by ending their visiting window —
// sets both VisitFrom and VisitTo to now, so the PIN still exists on the
// visitor record but the time window has closed and the PIN can't open
// any doors. The visitor stays in 2N with their PIN for quick re-extension
// next time (just set new VisitFrom/VisitTo + groups). Used by the revoke
// flow when the user chooses "Revoke access" (not "Delete visitor").
export async function revokeVisitorAccess(visitorId) {
  const now = new Date();
  const from = new Date(now.getTime() - 1000); // 1 second before now
  return await updateVisitorFields(visitorId, {
