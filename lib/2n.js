...
export async function updateVisitor({ visitorId, startDate, endDate, twoNGroupId, groupName }) {
  ...
  if (twoNGroupId) {
    body.Groups = [{ Id: twoNGroupId, Name: groupName || "" }];
  }
  return await putOrPatchVisitor(visitorId, body);
}

// Shared helper: tries PUT first, falls back to PATCH (2N API method
// is unverified — this covers both REST conventions). Throws with a
// clear error including both status codes if both fail.
async function putOrPatchVisitor(visitorId, body) {
  ...
  throw new Error(
    `2N update visitor failed. PUT: ${putRes.status} ${putText.slice(0, 200)}. PATCH: ${patchRes.status} ${patchText.slice(0, 200)}`
  );
}
...
// Deactivates a 2N visitor's access by setting their VisitTo to now,
// so the PIN stops working immediately. The visitor record stays in the
// 2N database for future re-extension. Used by the revoke flow when the
// user chooses "revoke access" (not "delete visitor").
export async function deactivateVisitor(visitorId) {
  const now = new Date().toISOString();
  return await putOrPatchVisitor(visitorId, { VisitTo: now });
}
